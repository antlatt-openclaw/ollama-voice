...
class VADProcessor:
    ...
    def get_speech_prob(self, pcm_bytes: bytes) -> float:
        ...
        if len(pcm_bytes) != EXPECTED_BYTES:
            ...
            print(f"[VAD] get_speech_prob: unexpected chunk size {len(pcm_bytes)} bytes (expected {EXPECTED_BYTES}), skipping")
            return 0.0
        try:
            ...
            print(f"[VAD] get_speech_prob error: {e}")
            return 0.0
    ...
    def load(self):
        try:
            ...
            if not os.path.exists(model_path):
                print("[SmartTurn] Downloading Smart Turn v3.2 model (~30 MB)...")
